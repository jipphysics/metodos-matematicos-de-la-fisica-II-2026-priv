...
Consider an $n$-dimensional manifold $D$ (the domain) and a function
$$
F:
...
$$
where $p_r \in \RR^{n^r}=\RR\times \RR\times ... \times \RR$ for $r=1,...,k$.
The equation
$$
F(x,v,p_1,p_2,...,p_k) = 0
$$
...
(p_k)_{i_1...i_k} = (\nabla_{i_1}...\nabla_{i_k} u)(x),
$$
for an arbitrary $x\in D$ and indices $i_1,...,i_k\in \{1,...,n\}$, defines a Partial Differential Equation of order $k$ for some solution function $u:D\ni x\to u(x)\in \RR$.
...
Namely, a given function $u:D\to \RR$ solves the PDE in the weak or distributional sense if the corresponding function $W_u:D\to \RR$ given by
$$
W_u(x):=F(x,u(x),(\nabla u)(x),...,(\nabla ... \nabla u)(x))
$$
...
:= 
\int_D dV\, W_u(x)\, \varphi(x)
$$
for every test function $\varphi$ in an appropriate test space $\Phi$.
Here, notice that the mapping $\Phi\ni \varphi\to \langle W_u, \varphi\rangle\in \RR$ defines a distribution in the corresponding space $\Phi'$.
