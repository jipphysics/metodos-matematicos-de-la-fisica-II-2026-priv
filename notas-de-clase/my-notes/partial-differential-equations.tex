...
Namely, a given function $u:D\to \RR$ solves the PDE in the weak or distributional sense if the corresponding function $G:D\to \RR$ given by
$$
G_u(x):=F(x,u(x),(\nabla u)(x),...,(\nabla ... \nabla u)(x))
$$
...
:= 
\int_D dV\, G(x)\, \varphi(x)
$$
for every test function $\varphi$ in an appropriate test space $\Phi$.
Here, notice that the mapping $\Phi\ni \varphi\to \langle G, \varphi\rangle\in \RR$ defines a distribution in the corresponding space $\Phi'$.
