...
Consider Partial Differential Equations (PDEs) defined by
\begin{equation}
\label{eq:X1}
a^{ij}(x,u(x),(\partial u)(x))
(
\partial_i
\partial_j
u)
...
0
\end{equation}
for all $x=(x^1,...,x^n)\in D\subset \RR^n$ with solutions $u:D\ni x \to u(x)\in \RR$.
Here  $(\partial u)(x) = ((\partial_1 u)(x),...,(\partial_n u)(x))\in \RR^n$.
Also, $f:D\times \RR\times \RR^n \ni (x,u(x),(\partial u)(x)) \to f(x,u(x),(\partial u)(x))\in \RR$.

For each $x\in D$ and solution $u$, the matrix of coefficients
$$
a^{ij}(x,u(x),(\partial u)(x))
\in \RR
$$
is assumed to be symmetric.
We will say that the PDE is:
\begin{itemize}
\item {\em elliptic} at $(x,u(x),(\partial u)(x))$ if the eigenvalues of the mentioned matrix are non-zero and of the same sign,

\item {\em hyperbolic} at $(x,u(x),(\partial u)(x))$ if the eigenvalues of the mentioned matrix are non-zero but of different signs, and

\item {\em parabolic} at $(x,u(x),(\partial u)(x))$ if at least one of the eigenvalues of the mentioned matrix is zero and the non-zero ones have the same sign.
\end{itemize}
...
\begin{equation}
\label{eq:X2}
a(x)u(x) + b(x)\frac{\partial u}{\partial \hat{n}}(x)
=
g(x)
\end{equation}
for $x\in \partial D$.
...
\hat{n}(x)\cdot (\nabla u)(x)
$$
where $\hat{n}$ is the unit outward normal vector to boundary $\partial D$ of $D$ at $x$.
...
\item {\em Robin} otherwise.
\end{itemize}
Also, the boundary condition is homogeneous if $g=0$, and non-homogeneous otherwise.

The Neumann boundary condition may not completely determine the solution and, at the same time, the boundary function $g$ cannot be completely arbitrary.
...
0
\end{equation}
holds for every test function $\varphi\in C_c^{\infty}(D)$.
For a given $u$, let
$$
A^{ij}(x)
:=
...
$$
\int_D A^{ij}\,\, \partial_i \partial_j u\,\, \varphi \,\, dV(x)
=
-
\int_D \partial_j u\,\, \partial_i(A^{ij} \varphi)\,\, dV(x)
+
\int_{\partial D} A^{ij}\,\,\partial_j u\,\, n_i\,\, \varphi \,\, dV(x).
$$
Here the last term vanishes because $\varphi$ has compact support on $D$.
Hence, we can rewrite Eq.~\ref{eq:X3} as
\begin{equation}
\label{eq:X4}
-
\int_D 
(\partial_j u)\,
\partial_i(A^{ij} \varphi)\,
dV(x)
+
\int_D
f\,
...
.
\end{equation}
We say that function $u$ satisfying Eq.~\ref{eq:X4} solves the PDE of Eq.~\ref{eq:X1} in a weak or distributional sense.

\subsection{Divergence form}

Consider functions $B^i:D\times \RR^n\ni (x,p) \to B^i(x,p)\in \RR$.
Then, for some given solution $u$ of Eq.~\ref{eq:X1}, define
$$
A^i(x) := B^i(x,(\partial u)(x)).
$$
Hence,
$$
(\partial_i A^i)(x)
=
\frac{\partial B^i}{\partial x^i}(x,(\partial u)(x))
+
\frac{\partial B^i}{\partial p_j}(x,(\partial u)(x))\,
(\partial_i \partial_j u)(x)
$$
Now, we can propose the identification
$$
a^{ij}(x,u(x),(\partial u)(x))
(\partial_i \partial_j u)(x)
=
