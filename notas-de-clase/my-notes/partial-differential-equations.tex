...
\begin{itemize}
\item {\em elliptic} at $(x,u(x),(\nabla u)(x))$ if the eigenvalues of the mentioned matrix are non-zero and of the same sign,

\item {\em hyperbolic} at $(x,u(x),(\nabla u)(x))$ if the eigenvalues of the mentioned matrix are non-zero but of different signs, and

\item {\em parabolic} at $(x,u(x),(\nabla u)(x))$ if at least one of the eigenvalues of the mentioned matrix is zero and the non-zero ones have the same sign.
\end{itemize}
...
\end{equation}
for $x\in \partial D$.
where $\hat{n}(x)$ is the unit outward normal vector to boundary $\partial D$ of $D$ at $x$.
...
$$
\int_D A^{ij}\,\, \partial_i \partial_j u\,\, \varphi \,\, dV
=
-
\int_D \partial_j u\,\, \partial_i(A^{ij} \varphi)\,\, dV
+
\int_{\partial D} A^{ij}\,\,\partial_j u\,\, n_i\,\, \varphi \,\, dS.
$$
...
Consider functions $B^i:D\times \RR \times \RR^n\ni (x,v,p) \to B^i(x,v,p)\in \RR$.
Then, for some given solution $u$ of Eq.~\ref{eq:X1}, define
$$
A^i(x) := B^i(x,u(x),(\partial u)(x)).
$$
Hence,
$$
(\partial_i A^i)(x)
=
\frac{\partial B^i}{\partial x^i}(x,u(x),(\partial u)(x))
+
\frac{\partial B^i}{\partial v}(x,u(x),(\partial u)(x))
\,
(\partial_i u)(x)
+
\frac{\partial B^i}{\partial p_j}(x,u(x),(\partial u)(x))\,
\,
(\partial_i \partial_j u)(x)
$$
Now, we propose the identification
$$
