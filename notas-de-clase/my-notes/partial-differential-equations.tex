...
Conversely, any $u$ satisfying Eq.~\ref{eq:X4} defines a distributional (or weak) solution of the PDE in the interior of $D$. 
...
The weak formulation can be written in a more standard and convenient form.
To this end, consider a given solution $u$ and let
$$
A^{ij}(x)
:=
a^{ij}(x,u(x),(\nabla u)(x))
.
$$
Then, after integrating by parts using as factors $A^{ij}\varphi$ and $\nabla_j u$, we obtain
$$
\int_D A^{ij}\,\, \nabla_i \nabla_j u\,\, \varphi \,\, dV
=
-
\int_D \nabla_j u\,\, \nabla_i(A^{ij} \varphi)\,\, dV
+
\int_{\partial D} A^{ij}\,\,\nabla_j u\,\, n_i\,\, \varphi \,\, dS.
$$
Here the last term vanishes because $\varphi$ has compact support on $D$~\footnote{Remember that $\varphi\in C_c^{\infty}$ means that the support of $\varphi$ (which is a closed set by definition, the closed set $\mathrm{sup}(\varphi)=\overline{\{x:\varphi(x)\neq 0\}}$) is fully contained in $D$ which is an open set by assumption.}.
Hence, Eq.~\ref{eq:X3} reduces to
%\begin{equation}
%\label{eq:X4}
$$
-
...
$$
%\end{equation}
Now we apply the Leibniz rule on $\nabla_i(A^{ij} \varphi)$ to obtain
$$
-
\int_{D}
A^{ij}\,
...
Supose there are functions $B^i:D\times \RR \times \RR^n\ni (x,v,p) \to B^i(x,v,p)\in \RR$ such that
\begin{equation}
\label{eq:X5}
a^{ij}
=
\frac{\partial B^i}{\partial p_j}.
\end{equation}
Then, for some given solution $u$ of Eq.~\ref{eq:X1}, define
$$
A^i(x) := B^i(x,u(x),(\partial u)(x)).
$$
as a function of $x\in D$.
Hence,
\begin{eqnarray}
(\partial_i A^i)(x)
&=&
...
\notag
\end{eqnarray}
so we can rewrite Eq.~\ref{eq:X1} as
\begin{equation}
\label{eq:X6}
(\partial_i A^i)(x)
+
\tilde{f}(x,u(x),(\partial u)(x))
=
0
\end{equation}
with
