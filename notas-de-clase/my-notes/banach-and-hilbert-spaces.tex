...
{\em Remark:} There are metric spaces that are not finite-dimensional.
...
\{y\in X: d(x,y)<r\}
$$
is called the {\em open ball} of $X$ centered at $x$ and of radious $r>0$.

\subsection{Open set}

A subset $U$ of $X$ is {\em open} if for every $x\in U$ there exists an open ball $B_r(x) \subset U$.

\subsection{Complement set}

Consider two subsets $Y$ and $Z$ of $X$ such that $Y\subset Z$.
The {\em complement} $Y\backslash Z$ of $Y$ in $Z$ is the subset of $X$ given by
$$
Y\backslash Z := \{y\in Y:y\notin Z\}.
$$

\subsection{Closed set}

A subset $F$ of $X$ is {\em closed} if its complement $F\backslash X$ is open.
...
A point $x$ is a {\em limit point} (or {\em accumulation point}) of a subset $A$ of $X$ if, for every $r>0$, it holds that
$$
...
A subset $A$ of $X$ is {\em bounded} if there exists a finite $r>0$ such that $A \subset B_r(0)$.

\subsection{Compact set}

Assume that $X$ is a finite-dimensional metric space.
A subset $K$ of $X$ is {\em compact} if $K$ is closed and bounded.

{\color{red} 
{\em Remark:}
In an infinite-dimensional metric space, compactness is defined differently and the definitions are not necessarily equivalent in all cases.
See~\url{https://chatgpt.com/share/6a0c5541-7498-83e9-a94f-8ca91aa5e791}.
}
...
The convergence of the sequence is abbreviately denoted by $x_j\to x$ as $j\to \infty$.
...
More formally, if $\{x_j\in X:j\in \NN\}$ is a Cauchy sequence, then there exists an $x\in X$ such that, for every $\epsilon>0$, there exists an $N\in \NN$ such that $d(x_j,x)<\epsilon$ for every $j\geq N$.
...
$$
Clearly, $x_j\to e$ as $j\to \infty$, i.e. it converges to Euler's number.
This is a Cauchy sequence in $\QQ$ but it does not converge to a value in $\QQ$ because $e$ is irrational.
It converges to a value in $\RR$.
Therefore, $\QQ$ is not a complete space.
...
\begin{enumerate}
\item triangular inequality, $|v+w|\leq |v|+|w|$,
...
d(v,w) = |v-w|.
$$
To see this, it is enough to check that it actually satisfies the triangle inequality because the other properties trivially follow.
Namely,
$$
