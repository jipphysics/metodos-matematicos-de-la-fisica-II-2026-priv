...
\{y\in X: d(x,y)<r\}
$$
is called the {\em open ball} of $X$ centered at $x$ and of radius $r>0$.
...
Consider two subsets $Y$ and $Z$ of $X$.
The {\em complement} $Y\backslash Z$ of $Y$ in $Z$ is the subset of $X$ given by
$$
Y\backslash Z := \{y\in Y:y\notin Z\}.
$$
In other words, by the elements in $Y$ that are not in $Z$.
In particular, $Y^c = X/Y$ is the subset of elements in $X$ that are not in $Y$.

\subsection{Closed set}

A subset $F$ of $X$ is {\em closed} if its complement $X\backslash F$ is open.
...
The {\em closure} $\overline{A}$ of a subset $A$ of $X$ is defined by
$$
\overline{A}
=
A \cup \{\mbox{all limit points of $A$}\}.
$$
...
A subset $A$ of $X$ is {\em bounded} if there exists an $x\in X$ and a finite $r>0$ such that $A \subset B_r(x)$.
...
The convergence of the sequence is abbreviated by $x_j\to x$ as $j\to \infty$.
