...
{\color{red}[Comment: Is this the Helmholtz equation?]}
...
Most one-dimensional eigenvalue problems can be written as
$$
-\frac{d}{dx}
\left(
p(x)\frac{dy}{dx}
...
n\neq m.
$$
{\color{red}[Comment: If the functions are complex, is the inner product modified?]}
...
{\color{red}[Comment: the use of $u$ in the following equations is confusing. Is this $u$ a function of $x$ alone, or is it also a function of $t$? In the latter case, the equation should hold for a particular $t$ or for all $t$?]}
...
$$
where $\hat n$ is the unit vector (pointing to the outside) normal to the boundary at each point $x\in \partial \Omega$.
...
$$
a u+b\frac{\partial u}{\partial \hat n}=g.
$$
...
\end{itemize}
Thus every separable PDE ultimately reduces to solving the same spatial eigenvalue problem.
{\color{red}[Comment: what do you mean by ``same'' here?]}
...
\right)^2.
$$
{\color{red}[Comment: Here $j_{mn}$ is the $n$-th root of the $m$-th Bessel function?]}
...
k_r^2+k_z^2.
$$
{\color{red}[Comment: The eigenvalues does not depend on $m$?]}
...
{\color{red}[Comment: Why the $j_{\ell}$ are written using a lower-case letter $j$? Is this correct?]}
...
{\color{red}[Comment: Isn't there a zero eigenvalue?]}

\section{Fourier Methods}

Useful on infinite domains.
{\color{red}[In infinite domains is useful the fourier transform? What about periodic spaces? What about Fourier series?]}
Transform
$$
\hat u(k)
=
...
{\color{red}[Comment: This is true given that there are no explicit powers of $x$ in the equation?]}
