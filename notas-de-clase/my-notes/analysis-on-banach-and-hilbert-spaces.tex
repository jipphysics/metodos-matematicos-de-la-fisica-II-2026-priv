...
Formally, that $\{f_n:n\in N\}$ is a Cauchy sequence under the uniform norm means that, for any $\epsilon>0$, there exists an $N\in \NN$ such that
$$
...
Now, it remains to show that the function $f:[a,b]\ni x\to f(x)\in \RR$ belongs to $C[a,b]$.
In other words, it remains to show that $f$ is continuous in $[a,b]$.
...
p(x)=a_0+a_1x+\cdots+a_nx^n
$$
with real coefficients $a_0,a_1,...,a_n$ such that:
$$
