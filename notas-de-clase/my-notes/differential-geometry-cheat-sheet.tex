...
{\em Remark:} Notice how the dependency of a derivation $v$ on the fact that is defined for a particular $p\in M$ is omitted in the notation for simplicity.
...
Given a function $f\in C^{\infty}(M)$ and a point $p\in M$, the differential of $f$ at $p$ is the map $df:T_pM\to \RR$ defined by
$$
df(v) = v(f)
$$
for each $v\in T_pM$.
Such map is linear since
$$
df(a\,v+b\,w)
=
(a\,v+b\,w)(f)
...
b\,df(w)
$$
for any $a,b\in \RR$ and $v,w\in T_pM$.
In fact, $df$ is an element of the dual space $T_p*M$ of $T_pM$ called the {\em cotangent space} of $M$ at $p$.
The covectors in $T_p^*M$ are called {\em 1-forms}.

{\em Remark:} Notice how the dependency of a 1-form $df$ on the fact that is defined for a particular $p\in M$ is omitted in the notation for simplicity.

\subsection{Cobasis}

Notice that $M\ni p\to x^i=\varphi^i(p)\in \RR$ defines a smooth function over $M$, i.e. an element of $C^{\infty}(M)$.
Therefore, the notation $dx^i$ is consistent with the notation $df$ for the particular case in which $f(p)=\varphi^i(p)=x^i$ for each $p\in M$.
...
$$
denote the tensor product of type $(r,s)$ at the point $p$ of $M$.
In particular, 
$$
T^{(0,0)}_pM=C^{\infty}(M),
$$
...
\subsection{Tensor coordinates}

In what follows we adopt Einstein's summation convention.
Given a tensor $t\in T_p^{(r,s)}M$ and a basis $\{\partial_1,...,\partial_n\}$ with corresponding dual basis $\{dx^1,...,dx^n\}$, we can always write
\begin{eqnarray}
t(\alpha_{1},...,\alpha_{r},v_1,...,v_s)
&=&
t((\alpha_{1})_{i_1}dx^{i_1},...,(\alpha_{1})_{i_r}dx^{i_r},(v_1)^{j_1}\partial_{j_1},...,(v_s)^{j_s}\partial_{j_s})
\notag
\\
&=&
(\alpha_{1})_{i_1}...\alpha_{1})_{i_r}(v_1)^{j_1}...(v_s)^{j_s}\,
t(dx^{i_1},...,dx^{i_r},\partial_{j_1},...,\partial_{j_s})
\notag
\\
&=&
(\alpha_{1})_{i_1}...\alpha_{1})_{i_r}(v_1)^{j_1}...(v_s)^{j_s}\,
...
where
$$
\alpha_k = (\alpha_k)_{i_k}\,dx^{i_k}
$$
denotes an arbitrary covector for $k=1,...,r$ and
$$
v_q = (v_q)^{j_q}\,\partial_{j_q}
$$
denotes an arbitrary vector for $q=1,...,s$.
We have also introduced the tensor coordinates
$$
t^{i_1...i_r}{}_{j_1...j_s}
:=
t(dx^{i_1},...,dx^{i_r},\partial_{j_1},...,\partial_{j_s})
...
A type $(0,2)$ tensor $g\in T^{(0,2)}_pM$ is called a metric if:
\begin{itemize}
\item It is symmetric, meaning $g(v,w)=g(w,v)$ for all $v,w\in T_pM$.
\item It is non-degenerate, meaning $g(v,w)=0$ for every $w\in T_pM$ implies $v=0$.
\item In some cases, we also say that it is positive definite, meaning that $g(v,v)\geq 0$ for all $v\in T_pM$.
\end{itemize}
In coordinates,
$$
g(v,w)
=
g(v^i\partial i,w^j \partial_j)
=
v^i w^j g(\partial i,\partial_j)
=
g_{ij}v^iw^j
$$
...
A metric $g$ defines an inner product on $T_pM$ for each $p\in M$.
Given a $v\in T_pM$, the mapping
$$
T_pM\ni w\to g(v,w)\in \RR
$$
defines a covector $\tilde{v}$ in $T_p^*M$.
Moreover, and without proof, the no-degeracy property of $g$ implies that for each covector $\alpha\in T_p^*M$, there exists an unique $v\in T_pM$ such that
$$
\alpha(w) = g(v,w) = \tilde{v}(w)
$$
for each $w\in T_pM$.
Therefore, the mapping $\Phi:T_pM\ni v\to \tilde{v}=\Phi(v)\in T_p^*M$ is not only linear, but also invertible.
In other words, this mapping establishes an isomorphism $\Phi$ between $T_pM$ and $T_p^*M$ called the {\em musical isomorphism}.
The inverse of such mapping is also an isomorphism and determines an unique vector $\tilde{\alpha} = \Phi^{-1}(\alpha)\in T_pM$ for each covector $\alpha\in T_p^*M$.
Furthermore, the mapping induces an inner product or metric $\tilde{g}$ over $T_p^*M$ given by
$$
\tilde{g}(\alpha,\beta) = g(\tilde{\alpha},\tilde{\beta})
$$
for each $\alpha,\beta\in T_p^*M$.

\subsection{Identification o $T_pM$ and $T_p^*M$}

Consider a vector $v\in T_pM$ and the corresponding covector $\tilde{v}=\Phi(v)$.
In a given basis, these have coordinates given by
$$
v = v^i\,\partial_i
$$
$$
\tilde{v} = \tilde{v}_j\,dx^j
$$
which are not independent but satisfy
$$
\tilde{v}_k
=
\tilde{v}_j \delta^j{}_k
=
\tilde{v}_j dx^j(\partial_k)
=
(\tilde{v}_j dx^j)(\partial_k)
=
\tilde{v}(\partial_k)
...
g(v,\partial_k)
=
g(v^i\partial_i,\partial_k)
=
v^i\,
g(\partial_i,\partial_k)
=
g_{ik}\,
v^i.
$$
Similarly, for any covector $\alpha\in T_p^*M$ and the corresponding vector $\tilde{\alpha}\in T_pM$ with coordinates given by
$$
\alpha = \alpha_j\,dx^j
$$
$$
\tilde{\alpha} = \tilde{\alpha}^j\,\partial_j
$$
we find these to be related by
$$
\tilde{\alpha}^k
=
\tilde{\alpha}^j\delta_j{}^k
=
\tilde{\alpha}^j\partial_j(dx^k)
=
(\tilde{\alpha}^j\partial_j)(dx^k)
=
\tilde{\alpha}(dx^k)
...
\tilde{g}(\alpha,dx^k)
=
\tilde{g}(\alpha_jdx^j,dx^k)
=
\alpha_j\tilde{g}(dx^j,dx^k)
=
\alpha_j\tilde{g}^{jk}
$$
where we have used in many places that $T_pM\cong T_p^{**}M$, i.e. that a vector space is naturally isomorphic to its double dual.
In particular, previous results imply that
$$
\tilde{g}^{ij}g_{jk} = \delta^i{}_k
\quad
\text{and}
\quad
g_{ij}\tilde{g}^{jk} = \delta_i{}^k.
$$


Since $T_pM$ and $T_p* M$ become isomorphic via $g$, there is no need to keep distinguishing vector from covectors, so we can conveniently simplify the notation.
Namely, we can identify each vector $v$ with its corresponding covector $\tilde{v}$ and vice-versa.
In this way, we say that $v$ has coordinates $v^i$ in the basis $\{\partial_1,...,\partial_n\}$ and coordinates $v^j$ in the corresponding cobasis $\{dx^1,...,dx^n\}$, and that these coordinates are related by
$$
v_i = g_{ij}\,v^j
\quad
\text{and}
...
$$
The same idea can be also applied to any tensor of any type.
